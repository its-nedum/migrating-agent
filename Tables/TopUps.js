...
const TopUps = async () => {
    ...
    try {
        ...
        if (fs.existsSync('topup.json')) {
            const progressData = fs.readFileSync('topup.json', 'utf8');
        ...
        // We use transactions to ensure that either all topups are inserted successfully or none of them are, maintaining data consistency
        await target_connection.query("START TRANSACTION");
        console.log("Trasaction Started!")

        // Fetch the structure of the 'topups' table from the source database
        const [sourceTableFields] = await source_connection.execute("SHOW COLUMNS FROM topups");
        ...
        const createTableQuery = `CREATE TABLE IF NOT EXISTS topups (${sourceTableFields.map(field => `${field.Field} ${field.Type}`).join(",")})`;
        ...
        do {
            try {
                // Fetch topups from the source database
                batches = await source_connection.execute("SELECT * FROM topups LIMIT ?, ?", [offset, BATCH_SIZE]);
                console.log("Fetched topup data with offset and batch size");
                ...
                const insertions = batches[0].map(async (topup) => {
                    try {
                        // Check if this topup already exists in the target database
                        const [existingUser] = await target_connection.execute("SELECT * FROM topups WHERE id = ?", [topup.id]);
                        console.log("Checked for existing topup!")
                        
                        if(existingUser.length === 0){
                            // Any data normalization can take place here before insertion
                            const columns = Object.keys(topup).join(', ');
                            const values = Object.values(topup).map(val => target_connection.escape(val)).join(', ');
                            const insert_query = `INSERT INTO topups (${columns}) VALUES (${values})`;
                ...
                fs.writeFileSync('topup.json', JSON.stringify(progress));
            } catch (batchError) {
