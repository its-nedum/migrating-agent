...
const Messages = async () => {
    ...
    try {
        ...
        if (fs.existsSync('message.json')) {
            const progressData = fs.readFileSync('message.json', 'utf8');
        ...
        // We use transactions to ensure that either all messages are inserted successfully or none of them are, maintaining data consistency
        await target_connection.query("START TRANSACTION");
        console.log("Trasaction Started!")

        // Fetch the structure of the 'messages' table from the source database
        const [sourceTableFields] = await source_connection.execute("SHOW COLUMNS FROM messages");
        ...
        const createTableQuery = `CREATE TABLE IF NOT EXISTS messages (${sourceTableFields.map(field => `${field.Field} ${field.Type}`).join(",")})`;
        ...
        do {
            try {
                // Fetch messages from the source database
                batches = await source_connection.execute("SELECT * FROM messages LIMIT ?, ?", [offset, BATCH_SIZE]);
                console.log("Fetched message data with offset and batch size");
                ...
                const insertions = batches[0].map(async (message) => {
                    try {
                        // Check if this message already exists in the target database
                        const [existingUser] = await target_connection.execute("SELECT * FROM messages WHERE id = ?", [message.id]);
                        console.log("Checked for existing message!")
                        
                        if(existingUser.length === 0){
                            // Any data normalization can take place here before insertion
                            const columns = Object.keys(message).join(', ');
                            const values = Object.values(message).map(val => target_connection.escape(val)).join(', ');
                            const insert_query = `INSERT INTO messages (${columns}) VALUES (${values})`;
                ...
                fs.writeFileSync('message.json', JSON.stringify(progress));
            } catch (batchError) {
