...
const Roles = async () => {
    ...
    try {
        ...
        if (fs.existsSync('role.json')) {
            const progressData = fs.readFileSync('role.json', 'utf8');
        ...
        // We use transactions to ensure that either all roles are inserted successfully or none of them are, maintaining data consistency
        await target_connection.query("START TRANSACTION");
        console.log("Trasaction Started!")

        // Fetch the structure of the 'roles' table from the source database
        const [sourceTableFields] = await source_connection.execute("SHOW COLUMNS FROM roles");
        ...
        const createTableQuery = `CREATE TABLE IF NOT EXISTS roles (${sourceTableFields.map(field => `${field.Field} ${field.Type}`).join(",")})`;
        ...
        do {
            try {
                // Fetch roles from the source database
                batches = await source_connection.execute("SELECT * FROM roles LIMIT ?, ?", [offset, BATCH_SIZE]);
                console.log("Fetched role data with offset and batch size");
                ...
                const insertions = batches[0].map(async (role) => {
                    try {
                        // Check if this role already exists in the target database
                        const [existingUser] = await target_connection.execute("SELECT * FROM roles WHERE id = ?", [role.id]);
                        console.log("Checked for existing role!")
                        
                        if(existingUser.length === 0){
                            // Any data normalization can take place here before insertion
                            const columns = Object.keys(role).join(', ');
                            const values = Object.values(role).map(val => target_connection.escape(val)).join(', ');
                            const insert_query = `INSERT INTO roles (${columns}) VALUES (${values})`;
                ...
                fs.writeFileSync('role.json', JSON.stringify(progress));
            } catch (batchError) {
