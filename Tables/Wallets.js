...
const Wallets = async () => {
    ...
    try {
        ...
        if (fs.existsSync('wallet.json')) {
            const progressData = fs.readFileSync('wallet.json', 'utf8');
        ...
        // We use transactions to ensure that either all wallets are inserted successfully or none of them are, maintaining data consistency
        await target_connection.query("START TRANSACTION");
        console.log("Trasaction Started!")

        // Fetch the structure of the 'wallets' table from the source database
        const [sourceTableFields] = await source_connection.execute("SHOW COLUMNS FROM wallets");
        ...
        const createTableQuery = `CREATE TABLE IF NOT EXISTS wallets (${sourceTableFields.map(field => `${field.Field} ${field.Type}`).join(",")})`;
        ...
        do {
            try {
                // Fetch wallets from the source database
                batches = await source_connection.execute("SELECT * FROM wallets LIMIT ?, ?", [offset, BATCH_SIZE]);
                console.log("Fetched wallet data with offset and batch size");
                ...
                const insertions = batches[0].map(async (wallet) => {
                    try {
                        // Check if this wallet already exists in the target database
                        const [existingUser] = await target_connection.execute("SELECT * FROM wallets WHERE id = ?", [wallet.id]);
                        console.log("Checked for existing wallet!")
                        
                        if(existingUser.length === 0){
                            // Any data normalization can take place here before insertion
                            const columns = Object.keys(wallet).join(', ');
                            const values = Object.values(wallet).map(val => target_connection.escape(val)).join(', ');
                            const insert_query = `INSERT INTO wallets (${columns}) VALUES (${values})`;
                ...
                fs.writeFileSync('wallet.json', JSON.stringify(progress));
            } catch (batchError) {
