...
const Admins = async () => {
    ...
    try {
        ...
        if (fs.existsSync('admin.json')) {
            const progressData = fs.readFileSync('admin.json', 'utf8');
        ...
        // We use transactions to ensure that either all admins are inserted successfully or none of them are, maintaining data consistency
        await target_connection.query("START TRANSACTION");
        console.log("Trasaction Started!")

        // Fetch the structure of the 'admins' table from the source database
        const [sourceTableFields] = await source_connection.execute("SHOW COLUMNS FROM admins");
        ...
        const createTableQuery = `CREATE TABLE IF NOT EXISTS admins (${sourceTableFields.map(field => `${field.Field} ${field.Type}`).join(",")})`;
        ...
        do {
            try {
                // Fetch admins from the source database
                batches = await source_connection.execute("SELECT * FROM admins LIMIT ?, ?", [offset, BATCH_SIZE]);
                console.log("Fetched admin data with offset and batch size");
                ...
                const insertions = batches[0].map(async (admin) => {
                    try {
                        // Check if this admin already exists in the target database
                        const [existingUser] = await target_connection.execute("SELECT * FROM admins WHERE id = ?", [admin.id]);
                        ...
                        if(existingUser.length === 0){
                            ...
                            const values = Object.values(admin).map(val => target_connection.escape(val)).join(', ');
                            const insert_query = `INSERT INTO admins (${columns}) VALUES (${values})`;
                ...
                fs.writeFileSync('admin.json', JSON.stringify(progress));
            } catch (batchError) {
