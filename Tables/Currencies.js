...
const Currencies = async () => {
    ...
    try {
        ...
        if (fs.existsSync('currency.json')) {
            const progressData = fs.readFileSync('currency.json', 'utf8');
        ...
        // We use transactions to ensure that either all currencies are inserted successfully or none of them are, maintaining data consistency
        await target_connection.query("START TRANSACTION");
        console.log("Trasaction Started!")

        // Fetch the structure of the 'currencies' table from the source database
        const [sourceTableFields] = await source_connection.execute("SHOW COLUMNS FROM Currencies");
        ...
        const createTableQuery = `CREATE TABLE IF NOT EXISTS Currencies (${sourceTableFields.map(field => `${field.Field} ${field.Type}`).join(",")})`;
        ...
        do {
            try {
                // Fetch currencies from the source database
                batches = await source_connection.execute("SELECT * FROM Currencies LIMIT ?, ?", [offset, BATCH_SIZE]);
                console.log("Fetched currency data with offset and batch size");
                ...
                const insertions = batches[0].map(async (currency) => {
                    try {
                        // Check if this currency already exists in the target database
                        const [existingUser] = await target_connection.execute("SELECT * FROM Currencies WHERE id = ?", [currency.id]);
                        console.log("Checked for existing currency!")
                        
                        if(existingUser.length === 0){
                            // Any data normalization can take place here before insertion
                            const columns = Object.keys(currency).join(', ');
                            const values = Object.values(currency).map(val => target_connection.escape(val)).join(', ');
                            const insert_query = `INSERT INTO Currencies (${columns}) VALUES (${values})`;
                ...
                fs.writeFileSync('currency.json', JSON.stringify(progress));
            } catch (batchError) {
