...
const Payments = async () => {
    ...
    try {
        ...
        if (fs.existsSync('payment.json')) {
            const progressData = fs.readFileSync('payment.json', 'utf8');
        ...
        // We use transactions to ensure that either all payments are inserted successfully or none of them are, maintaining data consistency
        await target_connection.query("START TRANSACTION");
        console.log("Trasaction Started!")

        // Fetch the structure of the 'payments' table from the source database
        const [sourceTableFields] = await source_connection.execute("SHOW COLUMNS FROM payments");
        ...
        const createTableQuery = `CREATE TABLE IF NOT EXISTS payments (${sourceTableFields.map(field => `${field.Field} ${field.Type}`).join(",")})`;
        ...
        do {
            try {
                // Fetch payments from the source database
                batches = await source_connection.execute("SELECT * FROM payments LIMIT ?, ?", [offset, BATCH_SIZE]);
                console.log("Fetched payment data with offset and batch size");
                ...
                const insertions = batches[0].map(async (payment) => {
                    try {
                        // Check if this payment already exists in the target database
                        const [existingUser] = await target_connection.execute("SELECT * FROM payments WHERE id = ?", [payment.id]);
                        console.log("Checked for existing payment!")
                        
                        if(existingUser.length === 0){
                            // Any data normalization can take place here before insertion
                            const columns = Object.keys(payment).join(', ');
                            const values = Object.values(payment).map(val => target_connection.escape(val)).join(', ');
                            const insert_query = `INSERT INTO payments (${columns}) VALUES (${values})`;
                ...
                fs.writeFileSync('payment.json', JSON.stringify(progress));
            } catch (batchError) {
