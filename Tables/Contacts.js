...
const Contacts = async () => {
    ...
    try {
        ...
        if (fs.existsSync('contact.json')) {
            const progressData = fs.readFileSync('contact.json', 'utf8');
        ...
        const createTableQuery = `CREATE TABLE IF NOT EXISTS contacts (${sourceTableFields.map(field => `${field.Field} ${field.Type}`).join(",")})`;
        ...
        do {
            try {
                ...
                console.log("Fetched contact data with offset and batch size");
                ...
                const insertions = batches[0].map(async (contact) => {
                    try {
                        ...
                        const [existingUser] = await target_connection.execute("SELECT * FROM contacts WHERE id = ?", [contact.id]);
                        ...
                        if(existingUser.length === 0){
                            ...
                            const insert_query = `INSERT INTO contacts (${columns}) VALUES (${values})`;
                ...
                fs.writeFileSync('contact.json', JSON.stringify(progress));
            } catch (batchError) {
