...
const ShortLinks = async () => {
    ...
    try {
        ...
        if (fs.existsSync('shortlink.json')) {
            const progressData = fs.readFileSync('shortlink.json', 'utf8');
        ...
        // We use transactions to ensure that either all shortlinks are inserted successfully or none of them are, maintaining data consistency
        await target_connection.query("START TRANSACTION");
        console.log("Trasaction Started!")

        // Fetch the structure of the 'shortlinks' table from the source database
        const [sourceTableFields] = await source_connection.execute("SHOW COLUMNS FROM shortlinks");
        ...
        const createTableQuery = `CREATE TABLE IF NOT EXISTS shortlinks (${sourceTableFields.map(field => `${field.Field} ${field.Type}`).join(",")})`;
        ...
        do {
            try {
                // Fetch shortlinks from the source database
                batches = await source_connection.execute("SELECT * FROM shortlinks LIMIT ?, ?", [offset, BATCH_SIZE]);
                console.log("Fetched shortlink data with offset and batch size");
                ...
                const insertions = batches[0].map(async (shortlink) => {
                    try {
                        // Check if this shortlink already exists in the target database
                        const [existingUser] = await target_connection.execute("SELECT * FROM shortlinks WHERE id = ?", [shortlink.id]);
                        console.log("Checked for existing shortlink!")
                        
                        if(existingUser.length === 0){
                            // Any data normalization can take place here before insertion
                            const columns = Object.keys(shortlink).join(', ');
                            const values = Object.values(shortlink).map(val => target_connection.escape(val)).join(', ');
                            const insert_query = `INSERT INTO shortlinks (${columns}) VALUES (${values})`;
                ...
                fs.writeFileSync('shortlink.json', JSON.stringify(progress));
            } catch (batchError) {
