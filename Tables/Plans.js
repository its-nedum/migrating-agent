...
const Plans = async () => {
    ...
    try {
        ...
        if (fs.existsSync('plan.json')) {
            const progressData = fs.readFileSync('plan.json', 'utf8');
        ...
        // We use transactions to ensure that either all plans are inserted successfully or none of them are, maintaining data consistency
        await target_connection.query("START TRANSACTION");
        console.log("Trasaction Started!")

        // Fetch the structure of the 'plans' table from the source database
        const [sourceTableFields] = await source_connection.execute("SHOW COLUMNS FROM plans");
        ...
        const createTableQuery = `CREATE TABLE IF NOT EXISTS plans (${sourceTableFields.map(field => `${field.Field} ${field.Type}`).join(",")})`;
        ...
        do {
            try {
                // Fetch plans from the source database
                batches = await source_connection.execute("SELECT * FROM plans LIMIT ?, ?", [offset, BATCH_SIZE]);
                console.log("Fetched plan data with offset and batch size");
                ...
                const insertions = batches[0].map(async (plan) => {
                    try {
                        // Check if this plan already exists in the target database
                        const [existingUser] = await target_connection.execute("SELECT * FROM plans WHERE id = ?", [plan.id]);
                        console.log("Checked for existing plan!")
                        
                        if(existingUser.length === 0){
                            // Any data normalization can take place here before insertion
                            const columns = Object.keys(plan).join(', ');
                            const values = Object.values(plan).map(val => target_connection.escape(val)).join(', ');
                            const insert_query = `INSERT INTO plans (${columns}) VALUES (${values})`;
                ...
                fs.writeFileSync('plan.json', JSON.stringify(progress));
            } catch (batchError) {
