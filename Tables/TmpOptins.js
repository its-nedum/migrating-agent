...
const TmpOptins = async () => {
    ...
    try {
        ...
        if (fs.existsSync('tmpoptin.json')) {
            const progressData = fs.readFileSync('tmpoptin.json', 'utf8');
        ...
        // We use transactions to ensure that either all tmpoptins are inserted successfully or none of them are, maintaining data consistency
        await target_connection.query("START TRANSACTION");
        console.log("Trasaction Started!")

        // Fetch the structure of the 'tmpoptins' table from the source database
        const [sourceTableFields] = await source_connection.execute("SHOW COLUMNS FROM tmpoptins");
        ...
        const createTableQuery = `CREATE TABLE IF NOT EXISTS tmpoptins (${sourceTableFields.map(field => `${field.Field} ${field.Type}`).join(",")})`;
        ...
        do {
            try {
                // Fetch tmpoptins from the source database
                batches = await source_connection.execute("SELECT * FROM tmpoptins LIMIT ?, ?", [offset, BATCH_SIZE]);
                console.log("Fetched tmpoptin data with offset and batch size");
                ...
                const insertions = batches[0].map(async (tmpoptin) => {
                    try {
                        // Check if this tmpoptin already exists in the target database
                        const [existingUser] = await target_connection.execute("SELECT * FROM tmpoptins WHERE id = ?", [tmpoptin.id]);
                        console.log("Checked for existing tmpoptin!")
                        
                        if(existingUser.length === 0){
                            // Any data normalization can take place here before insertion
                            const columns = Object.keys(tmpoptin).join(', ');
                            const values = Object.values(tmpoptin).map(val => target_connection.escape(val)).join(', ');
                            const insert_query = `INSERT INTO tmpoptins (${columns}) VALUES (${values})`;
                ...
                fs.writeFileSync('tmpoptin.json', JSON.stringify(progress));
            } catch (batchError) {
        ...
        fs.unlinkSync('tmpoptin.json');
    } catch (error) {
        console.log(error);
    }
}

module.exports = TmpOptins