...
const Senders = async () => {
    ...
    try {
        ...
        if (fs.existsSync('sender.json')) {
            const progressData = fs.readFileSync('sender.json', 'utf8');
        ...
        // We use transactions to ensure that either all senders are inserted successfully or none of them are, maintaining data consistency
        await target_connection.query("START TRANSACTION");
        console.log("Trasaction Started!")

        // Fetch the structure of the 'senders' table from the source database
        const [sourceTableFields] = await source_connection.execute("SHOW COLUMNS FROM senders");
        ...
        const createTableQuery = `CREATE TABLE IF NOT EXISTS senders (${sourceTableFields.map(field => `${field.Field} ${field.Type}`).join(",")})`;
        ...
        do {
            try {
                // Fetch senders from the source database
                batches = await source_connection.execute("SELECT * FROM senders LIMIT ?, ?", [offset, BATCH_SIZE]);
                console.log("Fetched sender data with offset and batch size");
                ...
                const insertions = batches[0].map(async (sender) => {
                    try {
                        // Check if this sender already exists in the target database
                        const [existingUser] = await target_connection.execute("SELECT * FROM senders WHERE id = ?", [sender.id]);
                        console.log("Checked for existing sender!")
                        
                        if(existingUser.length === 0){
                            // Any data normalization can take place here before insertion
                            const columns = Object.keys(sender).join(', ');
                            const values = Object.values(sender).map(val => target_connection.escape(val)).join(', ');
                            const insert_query = `INSERT INTO senders (${columns}) VALUES (${values})`;
                ...
                fs.writeFileSync('sender.json', JSON.stringify(progress));
            } catch (batchError) {
