...
const Subscriptions = async () => {
    ...
    try {
        ...
        if (fs.existsSync('subscription.json')) {
            const progressData = fs.readFileSync('subscription.json', 'utf8');
        ...
        // We use transactions to ensure that either all subscriptions are inserted successfully or none of them are, maintaining data consistency
        await target_connection.query("START TRANSACTION");
        console.log("Trasaction Started!")

        // Fetch the structure of the 'subscriptions' table from the source database
        const [sourceTableFields] = await source_connection.execute("SHOW COLUMNS FROM subscriptions");
        ...
        const createTableQuery = `CREATE TABLE IF NOT EXISTS subscriptions (${sourceTableFields.map(field => `${field.Field} ${field.Type}`).join(",")})`;
        ...
        do {
            try {
                // Fetch subscriptions from the source database
                batches = await source_connection.execute("SELECT * FROM subscriptions LIMIT ?, ?", [offset, BATCH_SIZE]);
                console.log("Fetched subscription data with offset and batch size");
                ...
                const insertions = batches[0].map(async (subscription) => {
                    try {
                        // Check if this subscription already exists in the target database
                        const [existingUser] = await target_connection.execute("SELECT * FROM subscriptions WHERE id = ?", [subscription.id]);
                        console.log("Checked for existing subscription!")
                        
                        if(existingUser.length === 0){
                            // Any data normalization can take place here before insertion
                            const columns = Object.keys(subscription).join(', ');
                            const values = Object.values(subscription).map(val => target_connection.escape(val)).join(', ');
                            const insert_query = `INSERT INTO subscriptions (${columns}) VALUES (${values})`;
                ...
                fs.writeFileSync('subscription.json', JSON.stringify(progress));
            } catch (batchError) {
