...
const PlatformTypes = async () => {
    ...
    try {
        ...
        if (fs.existsSync('platformtype.json')) {
            const progressData = fs.readFileSync('platformtype.json', 'utf8');
        ...
        // We use transactions to ensure that either all platformtypes are inserted successfully or none of them are, maintaining data consistency
        await target_connection.query("START TRANSACTION");
        console.log("Trasaction Started!")

        // Fetch the structure of the 'platformtypes' table from the source database
        const [sourceTableFields] = await source_connection.execute("SHOW COLUMNS FROM platformtypes");
        ...
        const createTableQuery = `CREATE TABLE IF NOT EXISTS platformtypes (${sourceTableFields.map(field => `${field.Field} ${field.Type}`).join(",")})`;
        ...
        do {
            try {
                // Fetch platformtypes from the source database
                batches = await source_connection.execute("SELECT * FROM platformtypes LIMIT ?, ?", [offset, BATCH_SIZE]);
                console.log("Fetched platformtype data with offset and batch size");
                ...
                const insertions = batches[0].map(async (platformtype) => {
                    try {
                        // Check if this platformtype already exists in the target database
                        const [existingUser] = await target_connection.execute("SELECT * FROM platformtypes WHERE id = ?", [platformtype.id]);
                        console.log("Checked for existing platformtype!")
                        
                        if(existingUser.length === 0){
                            // Any data normalization can take place here before insertion
                            const columns = Object.keys(platformtype).join(', ');
                            const values = Object.values(platformtype).map(val => target_connection.escape(val)).join(', ');
                            const insert_query = `INSERT INTO platformtypes (${columns}) VALUES (${values})`;
                ...
                fs.writeFileSync('platformtype.json', JSON.stringify(progress));
            } catch (batchError) {
        ...
        fs.unlinkSync('platformtype.json');
    } catch (error) {
        console.log(error);
    }
}
