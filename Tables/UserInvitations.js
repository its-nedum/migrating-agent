...
const UserInvitations = async () => {
    ...
    try {
        ...
        if (fs.existsSync('userinvitation.json')) {
            const progressData = fs.readFileSync('userinvitation.json', 'utf8');
        ...
        // We use transactions to ensure that either all userinvitations are inserted successfully or none of them are, maintaining data consistency
        await target_connection.query("START TRANSACTION");
        console.log("Trasaction Started!")

        // Fetch the structure of the 'userinvitations' table from the source database
        const [sourceTableFields] = await source_connection.execute("SHOW COLUMNS FROM userinvitations");
        ...
        const createTableQuery = `CREATE TABLE IF NOT EXISTS userinvitations (${sourceTableFields.map(field => `${field.Field} ${field.Type}`).join(",")})`;
        ...
        do {
            try {
                // Fetch userinvitations from the source database
                batches = await source_connection.execute("SELECT * FROM userinvitations LIMIT ?, ?", [offset, BATCH_SIZE]);
                console.log("Fetched userinvitation data with offset and batch size");
                ...
                const insertions = batches[0].map(async (userinvitation) => {
                    try {
                        // Check if this userinvitation already exists in the target database
                        const [existingUser] = await target_connection.execute("SELECT * FROM userinvitations WHERE id = ?", [userinvitation.id]);
                        console.log("Checked for existing userinvitation!")
                        
                        if(existingUser.length === 0){
                            // Any data normalization can take place here before insertion
                            const columns = Object.keys(userinvitation).join(', ');
                            const values = Object.values(userinvitation).map(val => target_connection.escape(val)).join(', ');
                            const insert_query = `INSERT INTO userinvitations (${columns}) VALUES (${values})`;
                ...
                fs.writeFileSync('userinvitation.json', JSON.stringify(progress));
            } catch (batchError) {
        ...
        fs.unlinkSync('userinvitation.json');
    } catch (error) {
        console.log(error);
    }
}
