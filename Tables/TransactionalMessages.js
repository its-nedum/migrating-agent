...
const TransactionalMessages = async () => {
    ...
    try {
        ...
        if (fs.existsSync('trasactional.json')) {
            const progressData = fs.readFileSync('trasactional.json', 'utf8');
        ...
        const [sourceTableFields] = await source_connection.execute("SHOW COLUMNS FROM TransactionalMessages");
        ...
        const createTableQuery = `CREATE TABLE IF NOT EXISTS TransactionalMessages (${sourceTableFields.map(field => `${field.Field} ${field.Type}`).join(",")})`;
        ...
        do {
            try {
                ...
                batches = await source_connection.execute("SELECT * FROM TransactionalMessages LIMIT ?, ?", [offset, BATCH_SIZE]);
                console.log("Fetched trasactional data with offset and batch size");
                ...
                const insertions = batches[0].map(async (trasactional) => {
                    try {
                        // Check if this trasactional already exists in the target database
                        const [existingUser] = await target_connection.execute("SELECT * FROM TransactionalMessages WHERE id = ?", [trasactional.id]);
                        console.log("Checked for existing trasactional!")
                        
                        if(existingUser.length === 0){
                            // Any data normalization can take place here before insertion
                            const columns = Object.keys(trasactional).join(', ');
                            const values = Object.values(trasactional).map(val => target_connection.escape(val)).join(', ');
                            const insert_query = `INSERT INTO TransactionalMessages (${columns}) VALUES (${values})`;
                ...
                fs.writeFileSync('trasactional.json', JSON.stringify(progress));
            } catch (batchError) {
        ...
        fs.unlinkSync('trasactional.json');
    } catch (error) {
        console.log(error);
    }
}

module.exports = TransactionalMessages