...
const Integrations = async () => {
    ...
    try {
        ...
        if (fs.existsSync('integration.json')) {
            const progressData = fs.readFileSync('integration.json', 'utf8');
        ...
        // We use transactions to ensure that either all integrations are inserted successfully or none of them are, maintaining data consistency
        await target_connection.query("START TRANSACTION");
        console.log("Trasaction Started!")

        // Fetch the structure of the 'integrations' table from the source database
        const [sourceTableFields] = await source_connection.execute("SHOW COLUMNS FROM integrations");
        ...
        const createTableQuery = `CREATE TABLE IF NOT EXISTS integrations (${sourceTableFields.map(field => `${field.Field} ${field.Type}`).join(",")})`;
        ...
        do {
            try {
                // Fetch integrations from the source database
                batches = await source_connection.execute("SELECT * FROM integrations LIMIT ?, ?", [offset, BATCH_SIZE]);
                console.log("Fetched integration data with offset and batch size");
                ...
                const insertions = batches[0].map(async (integration) => {
                    try {
                        // Check if this integration already exists in the target database
                        const [existingUser] = await target_connection.execute("SELECT * FROM integrations WHERE id = ?", [integration.id]);
                        console.log("Checked for existing integration!")
                        
                        if(existingUser.length === 0){
                            // Any data normalization can take place here before insertion
                            const columns = Object.keys(integration).join(', ');
                            const values = Object.values(integration).map(val => target_connection.escape(val)).join(', ');
                            const insert_query = `INSERT INTO integrations (${columns}) VALUES (${values})`;
                ...
                fs.writeFileSync('integration.json', JSON.stringify(progress));
            } catch (batchError) {
        ...
        fs.unlinkSync('integration.json');
    } catch (error) {
        console.log(error);
    }
}
