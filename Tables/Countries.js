...
const Countries = async () => {
    ...
    try {
        ...
        if (fs.existsSync('country.json')) {
            const progressData = fs.readFileSync('country.json', 'utf8');
        ...
        // We use transactions to ensure that either all countries are inserted successfully or none of them are, maintaining data consistency
        await target_connection.query("START TRANSACTION");
        console.log("Trasaction Started!")

        // Fetch the structure of the 'countries' table from the source database
        const [sourceTableFields] = await source_connection.execute("SHOW COLUMNS FROM countries");
        ...
        const createTableQuery = `CREATE TABLE IF NOT EXISTS countries (${sourceTableFields.map(field => `${field.Field} ${field.Type}`).join(",")})`;
        ...
        do {
            try {
                // Fetch countries from the source database
                batches = await source_connection.execute("SELECT * FROM countries LIMIT ?, ?", [offset, BATCH_SIZE]);
                console.log("Fetched country data with offset and batch size");
                ...
                const insertions = batches[0].map(async (country) => {
                    try {
                        // Check if this country already exists in the target database
                        const [existingUser] = await target_connection.execute("SELECT * FROM countries WHERE id = ?", [country.id]);
                        console.log("Checked for existing country!")
                        
                        if(existingUser.length === 0){
                            // Any data normalization can take place here before insertion
                            const columns = Object.keys(country).join(', ');
                            const values = Object.values(country).map(val => target_connection.escape(val)).join(', ');
                            const insert_query = `INSERT INTO countries (${columns}) VALUES (${values})`;
                ...
                fs.writeFileSync('country.json', JSON.stringify(progress));
            } catch (batchError) {
