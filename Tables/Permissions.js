...
const Permissions = async () => {
    ...
    try {
        ...
        if (fs.existsSync('permission.json')) {
            const progressData = fs.readFileSync('permission.json', 'utf8');
        ...
        // We use transactions to ensure that either all permissions are inserted successfully or none of them are, maintaining data consistency
        await target_connection.query("START TRANSACTION");
        console.log("Trasaction Started!")

        // Fetch the structure of the 'permissions' table from the source database
        const [sourceTableFields] = await source_connection.execute("SHOW COLUMNS FROM permissions");
        ...
        const createTableQuery = `CREATE TABLE IF NOT EXISTS permissions (${sourceTableFields.map(field => `${field.Field} ${field.Type}`).join(",")})`;
        ...
        do {
            try {
                ...
                batches = await source_connection.execute("SELECT * FROM permissions LIMIT ?, ?", [offset, BATCH_SIZE]);
                console.log("Fetched permission data with offset and batch size");
                ...
                const insertions = batches[0].map(async (permission) => {
                    try {
                        // Check if this permission already exists in the target database
                        const [existingUser] = await target_connection.execute("SELECT * FROM permissions WHERE id = ?", [permission.id]);
                        console.log("Checked for existing permission!")
                        
                        if(existingUser.length === 0){
                            // Any data normalization can take place here before insertion
                            const columns = Object.keys(permission).join(', ');
                            const values = Object.values(permission).map(val => target_connection.escape(val)).join(', ');
                            const insert_query = `INSERT INTO permissions (${columns}) VALUES (${values})`;
                ...
                fs.writeFileSync('permission.json', JSON.stringify(progress));
            } catch (batchError) {
