...
const Wallet = async () => {
    ...
    try {
        ...
        // We use transactions to ensure that either all wallet are inserted successfully or none of them are, maintaining data consistency
        await target_connection.query("START TRANSACTION");
        console.log("Trasaction Started!")

        // Fetch the structure of the 'wallet' table from the source database
        const [sourceTableFields] = await source_connection.execute("SHOW COLUMNS FROM wallet");
        ...
        const createTableQuery = `CREATE TABLE IF NOT EXISTS wallet (${sourceTableFields.map(field => `${field.Field} ${field.Type}`).join(",")})`;
        ...
        do {
            try {
                // Fetch wallet from the source database
                batches = await source_connection.execute("SELECT * FROM wallet LIMIT ?, ?", [offset, BATCH_SIZE]);
                ...
                const insertions = batches[0].map(async (wallet) => {
                    try {
                        ...
                        const [existingUser] = await target_connection.execute("SELECT * FROM wallet WHERE id = ?", [wallet.id]);
                        ...
                        if(existingUser.length === 0){
                            ...
                            const insert_query = `INSERT INTO wallet (${columns}) VALUES (${values})`;
