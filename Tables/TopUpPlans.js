...
const TopUpPlans = async () => {
    ...
    try {
        ...
        if (fs.existsSync('topupplan.json')) {
            const progressData = fs.readFileSync('topupplan.json', 'utf8');
        ...
        // We use transactions to ensure that either all topupplans are inserted successfully or none of them are, maintaining data consistency
        await target_connection.query("START TRANSACTION");
        console.log("Trasaction Started!")

        // Fetch the structure of the 'topupplans' table from the source database
        const [sourceTableFields] = await source_connection.execute("SHOW COLUMNS FROM topupplans");
        ...
        const createTableQuery = `CREATE TABLE IF NOT EXISTS topupplans (${sourceTableFields.map(field => `${field.Field} ${field.Type}`).join(",")})`;
        ...
        do {
            try {
                // Fetch topupplans from the source database
                batches = await source_connection.execute("SELECT * FROM topupplans LIMIT ?, ?", [offset, BATCH_SIZE]);
                console.log("Fetched topupplan data with offset and batch size");
                ...
                const insertions = batches[0].map(async (topupplan) => {
                    try {
                        // Check if this topupplan already exists in the target database
                        const [existingUser] = await target_connection.execute("SELECT * FROM topupplans WHERE id = ?", [topupplan.id]);
                        console.log("Checked for existing topupplan!")
                        
                        if(existingUser.length === 0){
                            // Any data normalization can take place here before insertion
                            const columns = Object.keys(topupplan).join(', ');
                            const values = Object.values(topupplan).map(val => target_connection.escape(val)).join(', ');
                            const insert_query = `INSERT INTO topupplans (${columns}) VALUES (${values})`;
                ...
                fs.writeFileSync('topupplan.json', JSON.stringify(progress));
            } catch (batchError) {
        ...
        fs.unlinkSync('topupplan.json');
    } catch (error) {
        console.log(error);
    }
}

module.exports = TopUpPlans